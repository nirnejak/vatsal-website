...
\hline
\textbf{Normalization Axis} & \text{Across batch (samples) for each feature} & \text{Across features for each sample} \\
\hline
\textbf{Best For} & \text{Image and non-sequential data} & \text{Sequential data, e.g., NLP and transformers} \\
\hline
\textbf{Batch Size Dependency} & \text{Requires batch statistics} & \text{Independent of batch size} \\
\hline
\textbf{Effect on Training} & \text{Speeds up training, stabilizes gradients} & \text{Adapts well to varying sequence lengths} \\
\hline
\end{array}
$$
