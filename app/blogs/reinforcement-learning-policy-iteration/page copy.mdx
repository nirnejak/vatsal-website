...
To answer that, we must teach it to **look beyond immediate rewards** — to think ahead, like a good chess player.

Imagine the drone is standing at **state 14**. It can either fly **right (to state 15)** or **left (to state 13)**. Both transitions give the same **immediate reward of −0.04**, so they look equally good at first glance. But intuition tells us something else: **state 15** is closer to the delivery rooftop (the green goal). If our drone only focuses on the immediate reward, it can’t distinguish between these options — it’s too **short-sighted.** To make wiser decisions, the drone must learn to consider the **long-term value** of each move — not just what happens *next.*
...
In the real world, **rewards in the future are less valuable than rewards right now.**

Our drone agrees — after all:
...
So instead of a single deterministic future, state 9 has a **lottery of futures**.

This means the utility of state 9 becomes:
$$
V(9)
= r(9) 
 + \gamma \Big(
...
$$
Now each state value includes *all* possible successor states, each weighted by its probability. 

To understand this better, recall that:
...
V(3)=r(3)
$$
The next step is to compute these utilities, but solving all of the linear equations at the same time in one big algebraic step would be very difficult. Instead, we use a dynamic programming method called iterative policy evaluation (or sweep), where we start by setting every state’s utility to zero and then loop through the states repeatedly to update them.
...
During each sweep, every state updates its value using the *most recent* values of its neighbors — this creates a smooth flow of information across the grid.

Eventually, after enough iterations, the numbers stop changing:
$$
...
The drone will simply replace the arrow at state 13 with the action that leads to the highest-valued neighbor.
...
But is this the **best** possible behavior? Not necessarily.
...
When we start Policy Evaluation the second time, we **don’t reset utilities to zero**.

We simply continue from the values we already computed. Since they’re already close to the truth, the second round converges much faster.
